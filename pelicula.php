...
$query3 = "SELECT C.`comentarioCritica`, C.`fechaCritica`, C.`Calificación`, U.`name`, U.`profile_pic` FROM `criticausuario` AS C JOIN `user` AS U ON C.`idUsuario` = U.`id` WHERE `idPelicula` = " . $idPelicula;
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-6 critica">
                            <table>
                                <th class="text-center crit-info">
                                    <?php echo $row['name'] ?>
                                    <img src="images/profiles/<?php echo $row['profile_pic']; ?>" alt="<?php echo $row['name']; ?>" class="img-circle profile_img">
